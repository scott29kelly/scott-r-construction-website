          ...
          {items.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  <span className="font-display text-card-heading text-navy max-md:text-[18px]">
                    {item.question}
                  </span>
                  <ChevronDown
                    size={20}
                    className={`shrink-0 text-navy/50 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}
                  />
                </button>
                <div
                  className={`overflow-hidden transition-all duration-200 ${isOpen ? 'max-h-[500px] pb-6' : 'max-h-0'}`}
                >
                  <p className="text-body-sm text-navy/70">{item.answer}</p>
                </div>
              </div>
            );
          })}
